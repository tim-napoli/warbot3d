...
void main (void) {
    ...
    if (light_states [0] == 1) {
        ...
        color +=  vec4 (gl_LightSource [0].diffuse *  gl_FrontMaterial.diffuse * lambertTerm) / light_att;
    }
    ...
    if (light_states [1] == 1) {
        ...
        L = normalize (Lt);
        lambertTerm = max (dot (Nn, L), 0.0);
        color +=  vec4 (gl_LightSource [1].diffuse *  gl_FrontMaterial.diffuse * lambertTerm) / light_att;
    }
    ...
    if (light_states [2] == 1) {
        ...
        L = normalize (Lt);
        lambertTerm = max (dot (Nn, L), 0.0);
        color +=  vec4 (gl_LightSource [2].diffuse *  gl_FrontMaterial.diffuse * lambertTerm) / light_att;
    }
    ...
    if (light_states [6] == 1) {
        ...
        color += gl_LightSource [6].ambient * gl_FrontMaterial.ambient;
    }
    ...
    if (light_states [7] == 1) {
        ...
        color += gl_LightSource [7].ambient * gl_FrontMaterial.ambient;
    }
